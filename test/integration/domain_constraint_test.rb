...
class DomainConstraintIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "routes to posts index with valid subdomain" do
    get "/", headers: {host: "#{@blog_with_subdomain.subdomain}.willow.camp"}
    assert_response :success
    assert_select "title", text: /#{@blog_with_subdomain.title}/i
  end

  test "routes to posts index with valid custom domain" do
    get "/", headers: {host: @blog_with_custom_domain.custom_domain}
    assert_response :success
  end

  test "routes to specific post with valid subdomain" do
    get "/#{@subdomain_post.slug}", headers: {host: "#{@blog_with_subdomain.subdomain}.willow.camp"}
    assert_response :success
  end

  test "routes to specific post with valid custom domain" do
    get "/#{@custom_domain_post.slug}", headers: {host: @blog_with_custom_domain.custom_domain}
    assert_response :success
  end

  test "routes to tags index with valid subdomain" do
    get "/tags", headers: {host: "#{@blog_with_subdomain.subdomain}.willow.camp"}
    assert_response :success
  end

  test "routes to tags index with valid custom domain" do
    get "/tags", headers: {host: @blog_with_custom_domain.custom_domain}
    assert_response :success
  end
  ...
  test "handles multi-level subdomains" do
    get "/", headers: {host: "blog.#{@blog_with_subdomain.subdomain}.willow.camp"}
    assert_response :not_found
  end

  test "redirects subdomain to custom domain when user has both" do
    get "/", headers: {host: "#{@blog_with_custom_domain.subdomain}.willow.camp"}
    assert_redirected_to "https://#{@blog_with_custom_domain.custom_domain}/"
  end

  test "redirects specific post from subdomain to custom domain" do
    get "/#{@custom_domain_post.slug}", headers: {host: "#{@blog_with_custom_domain.subdomain}.willow.camp"}
    assert_redirected_to "https://#{@blog_with_custom_domain.custom_domain}/#{@custom_domain_post.slug}"
  end

  test "redirects tags from subdomain to custom domain" do
    get "/tags", headers: {host: "#{@blog_with_custom_domain.subdomain}.willow.camp"}
    assert_redirected_to "https://#{@blog_with_custom_domain.custom_domain}/tags"
  end
  ...
  test "handles feed routes with subdomain" do
    get "/posts/rss", headers: {host: "#{@blog_with_subdomain.subdomain}.willow.camp"}
    assert_response :success
    assert_equal "application/rss+xml; charset=utf-8", response.content_type
  end

  test "handles feed routes with custom domain" do
    get "/posts/rss", headers: {host: @blog_with_custom_domain.custom_domain}
    assert_response :success
    assert_equal "application/rss+xml; charset=utf-8", response.content_type
  end

  test "redirects feed routes from subdomain to custom domain" do
    get "/posts/rss", headers: {host: "#{@blog_with_custom_domain.subdomain}.willow.camp"}
    assert_redirected_to "https://#{@blog_with_custom_domain.custom_domain}/posts/rss"
  end

  test "constraint respects database state changes" do
    # Update existing blog to have a different custom domain
    @blog_with_custom_domain.update!(custom_domain: "newblog.com")
    ...
    @blog_with_custom_domain.update!(custom_domain: nil)
    ...
    @blog_with_custom_domain.update!(custom_domain: "myblog.com")
  end

  test "handles domains with different ports" do
    get "/", headers: {host: "#{@blog_with_subdomain.subdomain}.willow.camp:3000"}
    assert_response :success
  end
  ...
  test "api routes work regardless of domain constraints" do
    ...
    get "/api/posts", headers: {host: "#{@blog_with_subdomain.subdomain}.willow.camp"}
    assert_response :unauthorized

    get "/api/posts", headers: {host: @blog_with_custom_domain.custom_domain}
    assert_response :unauthorized
  end

  test "dashboard routes work regardless of domain constraints" do
    sign_in @blog_with_subdomain.user

    get "/dashboard", headers: {host: "willow.camp"}
    assert_response :success

    get "/dashboard", headers: {host: "#{@blog_with_subdomain.subdomain}.willow.camp"}
    assert_response :success

    get "/dashboard", headers: {host: @blog_with_custom_domain.custom_domain}
    assert_response :success
  end
end
